...
const registerNewUser = AsyncHandler(async (req, res) => {
    ...
    if (user && !user.isVerified) {
        return res.status(HttpStatus.CONFLICT).json({
            message: "Account already exists. Login and Verify your account with OTP to continue.",
        });
    }
...
const resendNewOtp = AsyncHandler(async (req, res) => {
    ...
    if (!user || user.isVerified) {
        throw new ExpressError(HttpStatus.BAD_REQUEST, "If the account exists, an otp has been sent.");
    }
...
const verifyOtp = AsyncHandler(async (req, res) => {
    ...
    if (!realOtp || user.isVerified) {
        throw new ExpressError(HttpStatus.NOT_FOUND, "OTP expired. Request new OTP if possible.");
    }
    ...
    if (realOtp.attempts >= 5) {
        await EmailOtp.deleteOne({ _id: realOtp._id });
        throw new ExpressError(HttpStatus.FORBIDDEN, "Too many failed attempts. Request a new OTP.");
    }
...
const loginUser = AsyncHandler(async (req, res) => {
    ...
    const user = await User.findOne({ email });
    if (!user) {
        throw new ExpressError(HttpStatus.BAD_REQUEST, "Email or password is invalid");
    }
    ...
    if (!user.isVerified) {
        await createAndSendOtp(user);
        throw new ExpressError(HttpStatus.UNAUTHORIZED, "Verify your Email to continue. OTP sent to Email");
    }
    const token = signJwt({
...
const giveMyInfo = AsyncHandler(async (req, res) => {
